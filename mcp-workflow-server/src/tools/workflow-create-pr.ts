...
function extractIssueNumber(branchName: string): number | null {
  ...
  const patterns = [/issue-(\d+)/i, /\/#?(\d+)/, /-(\d+)$/, /phase-(\d+)/i];
...
export async function workflowCreatePR(
  input: CreatePRInput = {}
): Promise<WorkflowCreatePRResponse> {
  ...
  try {
    ...
    if (currentBranch === 'main' || currentBranch === 'master') {
      throw new Error(
        'Cannot create PR from default branch. Please create a feature branch first.'
      );
    }
    ...
    if (gitStatus.length > 0) {
      throw new Error(
        'You have uncommitted changes. Please commit or stash them before creating a PR.'
      );
    }
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Failed to push branch: ${pushError instanceof Error ? pushError.message : 'Unknown error'}`
        );
      }
    }
    ...
    const commits = execSync(
      `git log ${baseBranch}..HEAD --pretty=format:"%H%x00%s%x00%b%x00" --reverse`,
      { encoding: 'utf8' }
    )
      .trim()
      .split('\x00\n')
    ...
    if (commits.length === 0) {
      throw new Error(
        'No commits found between base branch and current branch. Make sure you have commits to create a PR.'
      );
    }
    ...
    try {
      ...
      automaticActions.push(
        `Could not auto-assign PR: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
    ...
    try {
      if (projectConfig.isComplete) {
        ...
        const itemId = (addResult as { addProjectV2ItemById: { item: { id: string } } })
      ...
      automaticActions.push(
        `Could not add PR to project board: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
