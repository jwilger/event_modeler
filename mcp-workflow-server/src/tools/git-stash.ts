...
export async function gitStash(input: GitStashInput): Promise<GitStashResponse> {
  ...
  try {
    switch (input.action) {
      ...
      case 'save': {
        ...
        try {
          ...
          suggestedActions.push(
            'Your changes have been saved. Use "git stash pop" to restore them later'
          );
      ...
      case 'apply': {
        ...
        automaticActions.push(
          `${input.action === 'pop' ? 'Popping' : 'Applying'} stash: ${stashRef}`
        );

        try {
          const output = execSync(command, { encoding: 'utf8' });
          automaticActions.push(
            `Stash ${input.action === 'pop' ? 'popped' : 'applied'} successfully`
          );
          ...
          if (error instanceof Error && error.message.includes('is not a stash reference')) {
            ...
            issuesFound.push(
              `Failed to ${input.action} stash: ${error instanceof Error ? error.message : String(error)}`
            );
          }
      ...
      case 'clear': {
        ...
        try {
          ...
          issuesFound.push(
            `Failed to clear stashes: ${error instanceof Error ? error.message : String(error)}`
          );
          return {
            requestedData: {
              stashDropped: false,
      ...
      case 'show': {
        ...
        try {
          ...
          issuesFound.push(
            `Failed to show stash: ${error instanceof Error ? error.message : String(error)}`
          );
          return {
    ...
    issuesFound.push(
      `Git stash operation failed: ${error instanceof Error ? error.message : String(error)}`
    );
    return {
      requestedData: { error: error instanceof Error ? error.message : String(error) },
      automaticActions,
