...
const RequestReviewInputSchema = z.object({
  ...
  skipBots: z.boolean().optional().describe('Skip bot reviewers like Copilot (default: true)'),
});
...
export async function workflowRequestReview(input: RequestReviewInput): Promise<WorkflowResponse> {
  ...
  try {
    ...
    automaticActions.push(`Found ${allReviewers.size} previous reviewers on PR #${prNumber}`);
    ...
    if (specificReviewers && specificReviewers.length > 0) {
      ...
      const notFound = specificReviewers.filter((login) => !allReviewers.has(login));
      if (notFound.length > 0) {
        issuesFound.push(`These reviewers were not found in PR history: ${notFound.join(', ')}`);
      }
    } else {
    ...
    reviewersToRequest = reviewersToRequest.filter((r) => !AUTO_REQUESTED_BOTS.includes(r.nodeId));
    ...
    for (const reviewer of reviewersToRequest) {
      try {
        ...
        if (errorMessage.includes('Review has already been requested')) {
          automaticActions.push(`${reviewer.login} already has a pending review request`);
          requestedReviewers.push(reviewer.login);
        } else if (errorMessage.includes('not a collaborator')) {
          failedRequests.push(`${reviewer.login} (not a collaborator)`);
        } else {
          failedRequests.push(`${reviewer.login} (${errorMessage})`);
        }
      }
    }

    if (failedRequests.length > 0) {
      issuesFound.push(`Failed to request reviews from: ${failedRequests.join(', ')}`);
    }

    if (requestedReviewers.length > 0) {
      suggestedActions.push(`Successfully requested ${requestedReviewers.length} review(s)`);
    }
  ...
  inputSchema: {
    ...
    properties: {
      ...
      reviewers: {
        ...
        items: { type: 'string' },
        description:
          'Specific reviewers to request (if empty, requests from all previous reviewers)',
      },
      skipBots: {
        type: 'boolean',
